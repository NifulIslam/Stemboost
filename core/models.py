...
class CartItem(models.Model):
    """A single course held in a learner's shopping cart."""
    learner    = models.ForeignKey(User, on_delete=models.CASCADE, related_name='cart_items')
    ...
    def __str__(self):
        return f'{self.learner.email} → cart → {self.course.title}'


# ── Enrollment ────────────────────────────────────────────────────────────────

class Enrollment(models.Model):
    """Records that a learner has access to (is enrolled in) a course."""
...
class Transaction(models.Model):
    """
    Records a demo/simulated purchase transaction.
    A single transaction may cover one or more courses (cart checkout).
    """
    ...
    learner         = models.ForeignKey(User, on_delete=models.CASCADE, related_name='transactions')
    courses         = models.ManyToManyField(Course, related_name='transactions', blank=True)
    ...
    def __str__(self):
        return f'Tx {self.transaction_ref} — {self.learner.email} — {self.status}'
